...
class CodeCollaborationService {
    ...
    static async leaveSession(sessionId, userId) {
        try {
            ...
            if (!session) {
                helpers_1.logger.error('Session not found or inactive:', { sessionId, userId });
                return false;
            }
            ...
            helpers_1.logger.error('Participant not found in session:', { sessionId, userId });
            return false;
        }
        catch (error) {
            helpers_1.logger.error('Error leaving session:', error);
    ...
    static async joinByInviteCode(inviteCode, userId) {
        try {
            ...
            const canJoin = session.isPublic ||
                session.invitedUsers?.includes(userId) ||
