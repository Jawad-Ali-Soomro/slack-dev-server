    ...
    this.io.on("connection", (socket) => {
      ...
      socket.on(
        ...
        (data: {
          ...
          logger.info(
            `User ${user.id} updated code in session ${data.sessionId}`,
          );
        },
      );
  ...
  private async authenticateSocket(socket: any, next: any): Promise<void> {
    try {
      ...
      try {
        ...
        logger.warn(
          "Socket token decryption failed, attempting raw token verification:",
          decryptError,
        );
      }
  ...
  public emitChatUpdate(chatId: string, chat: any): void {
    ...
    if (chat.participants?.length) {
      chat.participants.forEach((p: any) => {
        ...
        this.io.to(`user:${userId}`).emit("chat_updated", chat);
      });
    }
  }
